/*
    File: fn_createTrenchedVehicles.sqf
...
private _allVeh = [];
if (_sector isEqualTo "") exitWith {["No sector provided in fn_createTrenchedVehicles", "WARNING"] call KPLIB_fnc_log; _allVeh};
if (_objects isEqualTo []) exitWith {["No objects in the sector found", "SECTOR"] call KPLIB_fnc_log; _allVeh};
if (KPLIB_o_fixedUnits isEqualTo []) exitWith {_allVeh};

private _sectorpos = markerPos _sector;

// Find garrisons objects
//private _allGarrisons = (nearestObjects [_sectorpos, KPLIB_vehGarrisonConfigs_classes, _radius]) select {alive _x};
private _allGarrisons = _objects select {(typeOf _x) in KPLIB_vehGarrisonConfigs_classes};
			...
			if ((_x # 0 == typeOf _garrison)) then {
				...
				if (_positions > 0) then {
					for "_index" from 0 to (_positions - 1) do {
						...
						private _vehClass = selectRandom KPLIB_o_fixedVehicles;

                        if (isNil "_vehClass") then {["No fixed vehicle classname found in KPLIB_o_fixedVehicles", "WARNING"] call KPLIB_fnc_log; continue};

						_veh = [(_garrison modelToWorld _relPos), _vehClass, (getDir _garrison + (_relDir))] call KPLIB_fnc_spawnTrenchedVehicle;

						_allVeh pushBack _veh;
					};
				};
			}
		}forEach KPLIB_vehGarrisonConfigs;
	}forEach _allGarrisons;
};
