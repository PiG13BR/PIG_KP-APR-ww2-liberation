...
// WW2 mortar
if (_artyClass isKindOf "staticMortar") then {
	//_min = _min - 1000;
	_max = _max - 2000;
};
...
if !(_spawn_marker isEqualTo "") then {
	...
	// Barriers
	{
		...
		if (_x isKindOf "staticMortar") then {
			...
			// For heavy artillery
			private _barrier = createVehicle ["Land_fort_artillery_nest", markerPos "ghost_spot" , [], 0, "CAN_COLLIDE"];
			_barrier setPosATL (_x modelToWorld [0,2,0]);
